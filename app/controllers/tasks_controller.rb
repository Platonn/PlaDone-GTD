class TasksController < ApplicationController
  ...
  def index
    @tasks = @task_service.get_active_tasks_of(current_user)
  end
  ...
  def toggle_done
    if @task_service.toggle_done(@task_id, current_user)
  ...
  def update
    task_form = TaskForm.new(task_params)
    respond_to do |format|
      if @task_service.update(@task_id, task_form, current_user)
  ...
  def destroy
    ###Soft delete:
    @task_service.soft_delete(@task_id, current_user)
    respond_to do |format|
...
private
  ...
  def set_task
    ...
    raise ActionController::RoutingError.new('Given Task is null') if @task.nil?
  end
